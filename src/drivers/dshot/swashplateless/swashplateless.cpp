...
{
    float angle = (float)(pwm_now - pwm_min) / (float)(pwm_cap.width_range) * 360;
    if(angle > 360)
        angle = 360;
    else if(angle < 0)
        angle = 0;
...
void motor_status_logging(bool on_flag)
{
    frame_counter++;
    if(!on_flag)
    {
...
void mix_throttle(double sin_amp, double sin_pha, double throttle_common)
{
    ...
    double output_margin = CONTROL_MAX_THROTTLE - throttle_common;
...
uint16_t mixer_sa(bool on_flag)
{
    if(pwm_input_sub.update(&pwm_input_data))
    {
        ...
        motor_state.ref_x = single_axis.bat_vol_compen_coeff;//log for debug
        motor_status_logging(on_flag);
        #endif
    }
...
uint16_t throttle_rc(bool on_flag, bool bypass)
{
    if(pwm_input_sub.update(&pwm_input_data) || bypass)
    {
...
uint16_t speed_control_for_dshot(bool on_flag)
{
    ...
    return throttle_rc(on_flag, true); //for force/torque experiment or motor test
}
