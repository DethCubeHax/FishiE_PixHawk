...
#define CONTROL_MAX_THROTTLE (1800)
#define ACTUATOR_CONTROLS_TO_DSHOT (2000)



#define BODY_YAW_ANGLE_BIAS 1.53 //rad


//old
//#define CONST_MOMENT_TO_SIN_AMP 286
//#define CONST_SIN_AMP_LIMIT     400 //amp 400 = 0.82 Nm

//new:20210602
#define CONST_MOMENT_TO_SIN_AMP  (147)
...
// subscribe manual_control_setpoint (mapped rc data) from remote controller
manual_control_setpoint_s manual_control_setpoint_data{0};
...
uORB::Subscription actuator_controls_sub{ORB_ID(actuator_controls_1)};
...
pwm_encoder_me_s pwm_encoder_data = {0};
uORB::Publication<pwm_encoder_me_s> pwm_encoder_pub{ORB_ID(pwm_encoder_me)};
...
//butter,fc=20Hz,fs=910Hz，有轻微超调，后续稳定
//static float b[] = {0.0043, 0.0086, 0.0043};
//static float a[] = {1, -1.8052, 0.8226};

//butter,fc=10Hz,fs=910Hz，几乎无超调，后续很稳定（100RPM内）
static float b[] = {0.0011, 0.0022, 0.0011};
static float a[] = {1, -1.9024, 0.9070};

//chebyshev-1,fc=20Hz,fs=910Hz，有轻微超调，后续波动略大
//static float b[] = {0.0049, 0.0098, 0.0049};
//static float a[] = {1*0.8913, -1.8401*0.8913, 0.8596*0.8913};

//chebyshev-1,fc=10Hz,fs=910Hz，超调很大
//static float b[] = {0.0013, 0.0026, 0.0013};
//static float a[] = {1*0.8913, -1.922*0.8913, 0.9270*0.8913};

//butter,fc=10Hz,fs=833.33Hz，几乎无超调，后续很稳定（100RPM内）
//static float b[] = {0.0013, 0.0026, 0.0013};
//static float a[] = {1, -1.8935, 0.8989};
...
float motor_angle_cal(uint32_t pwm_now, uint32_t pwm_max, uint32_t pwm_min, float angle_bias, bool reverse)
{
    float width_range = float(pwm_cap.width_max - pwm_cap.width_min);
    float angle = float(pwm_now - pwm_min) / width_range * 360.f;
    if(angle > 360)
        angle = 360;
    else if(angle < 0)
        angle = 0;
...
//    usleep(1000);//tested: dshot module running at 1700Hz
    ...
    if(rc_channels_sub.update(&rc_data))
    {
        ...
        rc.LY = (double)rc_data.channels[2];//throttle, float value: 0 ~ 1, in my rc
        rc.LX = (double)rc_data.channels[3];//float value: -1 ~ 1, in my rc

        rc.switch_A = (int8_t)rc_data.channels[4];//index=4, but ch=5
...
//            speed_ref = RC_SWITCH_THROTTLE;
    ...
    if(manual_control_setpoint_sub.update(&manual_control_setpoint_data))
    {
        rc.throttle = manual_control_setpoint_data.z; //0.0~1.0
        rc.roll = manual_control_setpoint_data.y;     //-1.0~1.0
        rc.pitch = manual_control_setpoint_data.x;    //-1.0~1.0
        rc.yaw = manual_control_setpoint_data.r;      //-1.0~1.0
        //PX4_INFO("thr=%.3f, roll=%.3f, pitch=%.3f, yaw=%.3f", rc.throttle, rc.roll, rc.pitch, rc.yaw);
    }
...
uint16_t mixer_sa(bool on_flag)
{
...
uint16_t mixer_tailsitter(bool on_flag, uint16_t* value)
{
    if(pwm_input_sub.update(&pwm_input_data))
    {
        PX4_INFO("PWM INPUTS = %d", pwm_input_data.pulse_width);
    }

    PX4_INFO("pitch_controls = %.3f", single_axis.actuator_ctrls_pitch);
    value[0] = fabs(single_axis.actuator_ctrls_pitch) * 30;
    value[1] = fabs(single_axis.actuator_ctrls_pitch) * 30;
    value[0] = math::constrain(uint32_t(value[0]), 0u, 200u);
    value[1] = math::constrain(uint32_t(value[1]), 0u, 200u);
    return true;
}



//shaft from right to left (0-180deg), tail is left to right, fish will turn left
//shaft from left to right (180-360deg), tail is right to left, fish will turn right

#define GEAR_RATIO (91.f / 6.f)

#define SIN_AMP_LIMIT (150)

#define PWM_DOWN_LIMIT (1550)
#define PWM_UP_LIMIT (1650)//2000
#define PWM_DOWN_LIMIT_WITH_MARGIN (PWM_DOWN_LIMIT + 5)

uint16_t mixer_fish(bool on_flag, uint16_t* value)
{
    static int32_t last_counter = 0;
    static int64_t total_counter = 0;
    static double angle_total = 0, angle_one_rot = 0, angle_one_rot_reversed = 0;
    static int32_t current_counter = 0, delta_counter = 0;

    if(pwm_input_sub.update(&pwm_input_data))
    {
        //begin encoder angle conversion
        current_counter = int32_t(pwm_input_data.pulse_width);
        ...
        angle_total = double(total_counter) / (2000.0f * GEAR_RATIO) * 360.0f;//encoder line(500) * 4 * gear ratio(21 or 12.25, 91/6)
        angle_one_rot = fmod(angle_total, 360.0f);
        if (angle_one_rot < 0) angle_one_rot += 360.0f;
        //angle_one_rot_reversed = 360.0f - angle_one_rot;//reverse the angle due to rotation define
        angle_one_rot_reversed = angle_one_rot;  // no reverse
        if (angle_one_rot_reversed < 0) angle_one_rot_reversed += 360.0f;
        if (angle_one_rot_reversed > 360) angle_one_rot_reversed -= 360.0f;

        angle_one_rot = math::constrain((double)angle_one_rot, 0.0d, 360.0d);
        angle_one_rot_reversed = math::constrain((double)angle_one_rot_reversed, 0.0d, 360.0d);
        last_counter = current_counter;
        //end encoder angle conversion

//        PX4_INFO("counter = %d, angle_total = %.3f, angle_one_rot = %.3f, pub_counter = %d",
//                 pwm_input_data.pulse_width, angle_total, angle_one_rot, ++pub_counter);
    }

    //push up = -1
    //pull down = 1
    //middle is 0
    //move the tail to most left side (motor shaft is right side), and set the zero deg
    //look from tail, the rotation direction is anti-clockwise
    if (rc.switch_B > 0)
    {
        current_counter = 0;
        delta_counter = 0;
...
//    PX4_INFO("rc.switch_B = %d", rc.switch_B);



    // throttle calculation
    float throttle_range = fabs(float(PWM_UP_LIMIT - PWM_DOWN_LIMIT));
    float throttle_rc = rc.throttle * throttle_range;
    float pwm_thr = float(throttle_rc + PWM_DOWN_LIMIT);
    if (pwm_thr > PWM_UP_LIMIT) pwm_thr = PWM_UP_LIMIT;
    if (pwm_thr < PWM_DOWN_LIMIT) pwm_thr = PWM_DOWN_LIMIT;

    //sin value calculation
    float sin_amp = fabs(rc.yaw) * SIN_AMP_LIMIT;
    if (sin_amp > SIN_AMP_LIMIT) sin_amp = SIN_AMP_LIMIT;

    float sin_pha = 0;
    if (rc.yaw < -0.01) //stick to left
        sin_pha = 0.0f;
    if (rc.yaw > 0.01) //stick to right
        sin_pha = 180 / 57.2958f;

    float init_phase = 90.0f / 57.3f;

    //PX4_INFO("rc.yaw = %.3f", rc.yaw);

// calculate the sin_amp constrain
    float sin_down_bound = pwm_thr - PWM_DOWN_LIMIT_WITH_MARGIN;
    if (sin_down_bound < 0) sin_down_bound = 0;
    float sin_up_bound = PWM_UP_LIMIT - pwm_thr;
    if (sin_up_bound > PWM_UP_LIMIT) sin_up_bound = PWM_UP_LIMIT;
    float sin_bound = (sin_down_bound < sin_up_bound) ? sin_down_bound : sin_up_bound;
    //if (sin_amp > sin_bound) sin_amp = sin_bound;   //cancel the bound

    float pwm_sin = float(sin_amp * sin(angle_one_rot_reversed / 57.2958f - sin_pha + init_phase));

    //square wave
//    if (pwm_sin > 0) pwm_sin = sin_bound;
//    if (pwm_sin < 0) pwm_sin = -sin_bound;
    if (pwm_sin > 0) pwm_sin = sin_amp;
    if (pwm_sin < 0) pwm_sin = -sin_amp;


    float pwm_pitch = -rc.pitch * 500.0f;
    float pwm_roll = -rc.roll * 500.0f;
    //if (sin_amp > SIN_AMP_LIMIT / 2)
    if (0)
    {
        value[0] = math::constrain(uint32_t(pwm_thr + pwm_sin), (uint32_t)PWM_DOWN_LIMIT_WITH_MARGIN, (uint32_t)PWM_UP_LIMIT);
    }
    else
    {
        value[0] = math::constrain(uint32_t(pwm_thr + pwm_sin), (uint32_t)PWM_DOWN_LIMIT, (uint32_t)PWM_UP_LIMIT);
    }

    //PX4_INFO("pwm_motor = %d", value[0]);
    //servo is installed opposite, so pitch should be different sign
    //left servo connect to ch2, right servo connect to ch3
    value[1] = math::constrain(uint32_t(-pwm_pitch + pwm_roll + 1500.0f), 1000u, 2000u);
    value[2] = math::constrain(uint32_t(pwm_pitch + pwm_roll + 1500.0f), 1000u, 2000u);

//    PX4_INFO("pwm_thr = %.3f, sin_amp = %.3f, sin_bound = %.3f, pwm_sin = %.3f, angle_one_rot = %.3f, "
//             "thr1 = %d, thr2 = %d",
//             pwm_thr, sin_amp, sin_bound, pwm_sin, angle_one_rot, value[1], value[2]);

    static uint64_t last_time = hrt_absolute_time();
    uint64_t current_time = hrt_absolute_time();
    float dt = float(current_time - last_time) / 1000000.0f; //from micro sec to sec
    last_time = current_time;
    float motor_speed = delta_counter / (2000.0f * GEAR_RATIO) * 360.0f / dt;//angle per second

//add pub data for log
    pwm_encoder_data.timestamp = hrt_absolute_time();
    pwm_encoder_data.counter_reg = current_counter;
    pwm_encoder_data.counter_total = total_counter;
    pwm_encoder_data.counter_delta = delta_counter;
    pwm_encoder_data.angle_total = angle_total;
    pwm_encoder_data.angle_one_rot = angle_one_rot_reversed;
    pwm_encoder_data.motor_speed = motor_speed;

    pwm_encoder_data.pwm_thr = pwm_thr;
    pwm_encoder_data.pwm_sin = pwm_sin;
    pwm_encoder_data.pwm_total = value[0];
    pwm_encoder_data.sin_amp = sin_amp;
    pwm_encoder_pub.publish(pwm_encoder_data);
//end pub
...
#define TEST_UP_BOUND (8)
uint16_t throttle_rc2(bool on_flag)
{
//    double throttle = abs(rc.LY) * 1900 + 100;
    double throttle = 100;
    ...
    rc.switch_D_last = rc.switch_D;
    if (rc.switch_D > 0)//rc directly control, switch to down
    {
        float t_interval = ((float)(hrt_absolute_time() - inject_begin_timestamp)) / 1000000.0;
        int times = int(t_interval / 1.5) + 1;
        if (times > TEST_UP_BOUND) times = 4;
        throttle = times * 200;
        if (throttle > (TEST_UP_BOUND * 200)) throttle = (TEST_UP_BOUND * 200);
    }
    else
    {
        throttle = speed_ref + 100;
    }
    throttle = math::constrain(throttle, (double)0, (double)THROTTLE_MAX);
...
{
    update_orb_topic();
    //return mixer_sa(on_flag); //for sa fly control
    //return throttle_rc(on_flag); //for force/torque experiment or motor test manually
    //return throttle_rc2(on_flag); //for thrust auto test
}

void update_swashplateless_control(bool on_flag, uint16_t* throttle)
{
    update_orb_topic();
    //mixer_tailsitter(on_flag, throttle);
    mixer_fish(on_flag, throttle);
}
